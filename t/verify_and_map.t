...
#verify
my $verified =
  $mailmap->verify( 'proper-email' => '<santa.claus@northpole.xx>' );
...
is_deeply(
    \@proper,
    [ undef, '<cto@company.xx>' ],
    'Mapped <cto@company.xx> to <cto@company.xx>.'
);
@proper = $mailmap->map( 'email' => '<cto@coompany.xx>' );
is_deeply(
    \@proper,
    [ undef, '<cto@company.xx>' ],
    'Mapped <cto@coompany.xx> to <cto@company.xx>.'
);
@proper = $mailmap->map( 'email' => '<some@dude.xx>' );
is_deeply(
    \@proper,
    [ 'Some Dude', '<some@dude.xx>' ],
    'Mapped <some@dude.xx> to Some Dude.'
);
...
is_deeply(
    \@proper,
    [ undef, undef ],
    'Not mapped <not_mapped_address@address>.'
);
@proper = $mailmap->map( 'email' => 'faulty_email_address>' );
is_deeply( \@proper, [ undef, undef ], 'Not mapped "faulty_email_address>".' );
dies_ok { @proper = $mailmap->map( 'email' => '' ) }
'Failed when empty email address string.';
